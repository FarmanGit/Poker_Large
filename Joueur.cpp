...
using namespace std;
    ...
    {
        cout << nom_ << " wins " << jetons << " chips."<<endl;
        jetons_+=jetons;
    }
    ...
    int Joueur::miser(int mise)
    {
        if(laPartie_->validMise(mise+miseEnCours_))
        {
            if (mise>jetons_)
            {
                if (jetons_!=0)
                {
                    cout << nom_ << " wants to bet " << mise << " but has only " << jetons_ << " and bets all in" << endl;
                    int temp=jetons_;
            ...
            {
                cout << nom_ << " bets " << mise << endl;
                jetons_-=mise;
                miseEnCours_+=mise;
                cout << "Player betting : "<<miseEnCours_<<endl;
                return mise;
            }
        }
        else
    ...
    //se charge egalement de l'arrichage du changement d'�tat
    void Joueur::setEtat(State *etat)
    {
        ...
        {
            cout << nom_ << " fold" << endl;
            cout << "Player(s) in the game : " << laPartie_->joueursEnJeu() << endl;
    ...
    int Joueur::suivre(bool suivi)
    {
        if(suivi)
        {
            ...
            if (miseMax>jetons_)
            {
                cout << nom_ << " Wants to check " << miseMax << " but has only " << jetons_ <<endl;
                int temp=jetons_;
                jetons_=0;
                miseEnCours_+=temp;
                return temp;
            ...
            {
                cout << nom_ << " has checked with " << miseMax << endl;
                jetons_-=miseMax;
